...
namespace Prova1
{
	...
	class Controle
	{
		...
		public string GetEmpresasSalgadoManha()
		{
            ...
            foreach (var quiosque in Quiosques)
            {
                if(quiosque.ServeSalgado == true && quiosque.Horario == "Manha"){
                    listaQuiosques = quiosque.Empresa + ", " + listaQuiosques;
                }
            }
		...
		public string GetAlimentoMaisPreferido()
		{
            ...
            {
                switch (bloco.AlimentoPreferido)
                {
                    case "Salgado":
                        salgado += 1;
                        break;
            ...
            if(salgado > doce && salgado > bebida){
                maisPreferido = "Salgado";
            }
            else if(doce > salgado && doce > bebida){
                maisPreferido = "Doce";
            }else
            {
                maisPreferido = "Bebida";
            }
		...
		public void AlocarTodoMundo()
		{
			foreach (var bloco in Blocos)
            {
                ...
                foreach (var quiosque in Quiosques)
                {
                    if(bloco.Horario == quiosque.Horario || quiosque.Horario == "Ambos")
                    {
                        if(bloco.NumeroQuiosques < 2 && quiosque.ServeSalgado == true && bloco.AlimentoPreferido == "Salgado"){
                            Alocacoes.Add(new Alocacao(quiosque.Empresa, bloco.Cor));
                            bloco.NumeroQuiosques++;
                        }else if(bloco.NumeroQuiosques < 2 && quiosque.ServeDoce && bloco.AlimentoPreferido == "Doce"){
                            Alocacoes.Add(new Alocacao(quiosque.Empresa, bloco.Cor));
                            bloco.NumeroQuiosques++;
                        }else if(bloco.NumeroQuiosques < 2 && quiosque.ServeBebida && bloco.AlimentoPreferido == "Bebida"){
                            Alocacoes.Add(new Alocacao(quiosque.Empresa, bloco.Cor));
                            bloco.NumeroQuiosques++;
                        }
                    }
